export default function CookieForm({ handleSubmit }) {

  return (
    <form onSubmit={handleSubmit} className="flex flex-col w-3/5 p-10 mx-auto my-4 mt-5 border border-green-900 rounded-md bg-form-input-box-green">
      ...
      <div className="flex flex-row mt-6">
        <div className="flex flex-row w-full h-20">
          ...
          <input className="flex w-full mt-2 h-7 ml-2" name="location" placeholder="Cookie Stand Location" />
        </div>
      </div>
      <div className="flex flex-row items-center justify-between w-full h-10 mt-2 mb-2 space-x-4 ">
        ...
        <button className="w-64 rounded-sm h-11 bg-button-green align-text-middle">CREATE</button>
      </div>
    </form>
  );
}
